...
class Preparer
{
    ...
    public function prepare(EmailComposer $composer)
    {
        ...
        $this->prepareComposedBy($composer);

        $this->prepareCampaign($composer);

        $this->prepareEmailCampaign($composer);

        $this->prepareCompany($composer);
    }

    /**
     * Prepare the composed By for database storage.
     *
     * @param EmailComposer $composer
     */
    private function prepareComposedBy(EmailComposer $composer)
    {
        if (!$composer->hasData('composed_by_id')) {
            if (!empty(Auth::check())) {
                $composer->composedById(Auth::user()->id);
            }
        }

        $composer->getEmail()->fill([
            'composed_by_id' => $composer->getData('composed_by_id'),
        ]);
    }

    /**
     * Prepare the email campaign id for database storage.
     * @param EmailComposer $composer
     */
    private function prepareEmailCampaign(EmailComposer $composer)
    {
        if (!$composer->hasData('email_campaign_id')) {
            return;
        }

        $composer->getEmail()->fill([
            'email_campaign_id' => $composer->getData('email_campaign_id'),
        ]);
    }

    /**
     * Prepare the email company id for database storage.
     * @param EmailComposer $composer
     */
    private function prepareCompany(EmailComposer $composer)
    {
        if (!$composer->hasData('company_id')) {
            return;
        }

        $composer->getEmail()->fill([
            'company_id' => $composer->getData('company_id'),
        ]);
    }
     ...
     * @param EmailComposer $composer
     */
    private function prepareCampaign(EmailComposer $composer)
    {
        if (!$composer->hasData('campaign_id')) {
            return;
        }

        $composer->getEmail()->fill([
            'campaign_id' => $composer->getData('campaign_id'),
        ]);
    }
