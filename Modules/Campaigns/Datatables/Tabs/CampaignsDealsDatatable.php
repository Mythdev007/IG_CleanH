...
/**
 * Class CampaignsDealsDatatable
 * @package Modules\Campaigns\Datatables\Tabs
 */
class CampaignsDealsDatatable extends RelationDataTable
{
    const SHOW_URL_ROUTE = 'deals.deals.show';

    protected $unlinkRoute = 'campaigns.deals.unlink';
    ...
    public function dataTable($query)
    {
        ...
        $dataTable->filterColumn('closing_date', function ($query, $keyword) {
            $dates = DataTableHelper::getDatesForFilter($keyword);

            if ($dates != null) {
                $query->whereBetween('deals.closing_date', array($dates[0], $dates[1]));
            }
        });
