...
class CampaignDemoSeederTableSeeder extends SeederHelper
{
    ...
    public function run()
    {
        ...
        for ($i = 1; $i <= 20; $i++) {
            ...
            $campaign->campaign_status_id = CampaignStatus::where('company_id',$this->firstCompany())->get()->random(1)->first()->id;
            $campaign->campaign_type_id = CampaignType::where('company_id',$this->firstCompany())->get()->random(1)->first()->id;
        ...
        for ($i = 21; $i <= 40; $i++) {
            ...
            $campaign->campaign_status_id = CampaignStatus::where('company_id',$this->secondCompany())->get()->random(1)->first()->id;
            $campaign->campaign_type_id = CampaignType::where('company_id',$this->secondCompany())->get()->random(1)->first()->id;;
            $campaign->budget_cost = rand(5000, 65400);
            $campaign->actual_budget = rand(5000, 50000);
            $campaign->expected_response = rand(4000, 35959);
            $campaign->expected_revenue = rand(344, 4000);
