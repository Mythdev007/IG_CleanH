...
class CalendarViewController extends AppBaseController
{
    ...
    public function manageEvent(Request $request)
    {
        ...
        $startDate = UserHelper::parseDateFromString($request->get('startDate'));
        $endDate = UserHelper::parseDateFromString($request->get('endDate'));
        ...
        if ($action == CalendarViewController::ACTION_RESIZE) {
            $event = $this->eventRepo->update(['end_date' => $endDate], $event->id); // How to skip timezone
        ...
        if ($action == CalendarViewController::ACTION_DROP) {
            $event = $this->eventRepo->update(['start_date' => $startDate, 'end_date' => $endDate, 'full_day' => $fullDay], $event->id); // How to skip timezone
