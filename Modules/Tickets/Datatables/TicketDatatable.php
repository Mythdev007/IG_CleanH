...
class TicketDatatable extends PlatformDataTable
{
    ...
    public static function availableQueryFilters()
    {
        ...
        return [
            ...
            [
                'id' => 'tickets.ticket_owner_id',
                'label' => trans('tickets::tickets.form.ticket_owner_id'),
                'type' => 'integer',
                'input' => 'select',
                'multiple' => true,
                'plugin' => 'select2',
                ...
                'values' => FormHelper::shareWithChoises(),
                'operators' => [
                    'in',
                    'not_in',
                    'is_null',
    ...
    public static function availableColumns()
    {
        return [
            ...
            'ticket_owner' => [
                'name' => 'users.name',
                'data' => 'ticket_owner',
                'title' => trans('tickets::tickets.form.ticket_owner_id'),
                'data_type' => 'text',
                'filter_type' => 'text'
            ],
            'severity' => [
    ...
    public function dataTable($query)
    {
        ...
        $dataTable->filterColumn('owner', function ($query, $keyword) {
            DataTableHelper::queryOwner($query, $keyword,'tickets');
        });

        $dataTable->editColumn('priority', function ($record) {
            $text = "<span class='badge badge-default $record->priority_color'>$record->priority</span>";
            return DataTableHelper::applyLink($text,route(self::SHOW_URL_ROUTE,$record->id));
        });
        $dataTable->editColumn('severity', function ($record) {
            $text = "<span class='badge badge-default $record->severity_color'>$record->severity</span>";
            return DataTableHelper::applyLink($text,route(self::SHOW_URL_ROUTE,$record->id));
        });
    ...
    public function query(Ticket $model)
    {
        $query = $model->with('owner')
            ...
            ->leftJoin('users', 'tickets.ticket_owner_id', '=', 'users.id')
            ->newQuery()->select([
