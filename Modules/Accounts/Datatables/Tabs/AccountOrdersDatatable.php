...
/**
 * Class AccountOrdersDatatable
 * @package Modules\Accounts\Datatables\Tabs
 */
class AccountOrdersDatatable extends RelationDataTable
{
    const SHOW_URL_ROUTE = 'orders.orders.show';

    protected $unlinkRoute = 'accounts.orders.unlink';
    ...
    public function query(Order $model)
    {
        $query = $model->newQuery()
            ...
            ->leftJoin('vaance_currency','orders.currency_id','=','vaance_currency.id')
            ->leftJoin('vaance_tax','orders.tax_id','=','vaance_tax.id')
            ->select(
                'orders.*',
