    ...
    public function query(User $model)
    {
        ...
        if(\Auth::user()->hasPermissionTo('settings.access')){
            $query->where(function ($q){
                $q->where('company_id',\Landlord::getTenantId('company_id'));
                $q->orWhereIn('id',User::role(User::ADMIN_ROLE_NAME)->get()->pluck('id'));
            });

        }else{
            $query->where('company_id',\Landlord::getTenantId('company_id'));
