...
class UserController extends SettingsCrudController
{
    ...
    protected $showFields = [
        'login_and_role' => [
            ...
            'first_name' => ['type' => 'text', 'col-class' => 'col-lg-3 col-md-3 col-sm-12'],
            'last_name' => ['type' => 'text', 'col-class' => 'col-lg-3 col-md-3 col-sm-12'],
            'email' => ['type' => 'email', 'col-class' => 'col-lg-3 col-md-3 col-sm-12'],
            'password' => ['type' => 'password', 'col-class' => 'col-lg-3 col-md-3 col-sm-12', 'hide_in_edit' => true],
            'company_id' => ['type' => 'manyToOne', 'relation' => 'company', 'column' => 'name', 'col-class' => 'col-lg-4 col-md-4 col-sm-12', 'permission' => 'settings.access'],
            'is_active' => ['type' => 'checkbox', 'col-class' => 'col-lg-4 col-md-4 col-sm-12'],
            'access_to_all_entity' => ['type' => 'checkbox', 'col-class' => 'col-lg-4 col-md-4 col-sm-12'],
            'roles' => ['type' => 'oneToMany', 'relation' => 'roles', 'column' => 'display_name', 'col-class' => 'col-lg-4 col-md-4 col-sm-12 clear-both'],
        ],
        'address' => [
    ...
    public function __construct(RoleRepository $repository, UserService $userService)
    {
        parent::__construct();
    ...
    public function show($identifier)
    {
        if (!\Auth::user()->hasPermissionTo('settings.access')) {
            $user = \App::make($this->repository)->findWithoutFail($identifier);
    ...
    public function store()
    {
        ...
        $passwordTemp = UserHelper::randomPassword(6);

        if (empty($input['password'])) {

            $passwordTemp = UserHelper::randomPassword(6);

            $input['password'] = bcrypt($passwordTemp);
        } else {
            $passwordTemp = $input['password'];
            $input['password'] = bcrypt($input['password']);
        }

        if (!$user->isAdmin() && !empty($user->company)) {
        ...
        if (is_null($request->get('roles'))) {
            ...
            if (\Auth::user()->hasPermissionTo('settings.access')) {
                $roles = $this->roleRepo->getRolesByNames($request->get('roles'), true);
                ...
                $roles = $this->roleRepo->getRolesByNames($request->get('roles'));
                $entity->syncRoles($roles);
            }
        ...
        $this->userService->sendInviteEmail($entity,$passwordTemp);

        flash(trans($this->languageFile . '.created', ['password' => $passwordTemp]))->success();
    ...
    protected function setupCustomButtons()
    {
        ...
        $this->customShowButtons[] = array(
            'attr' => [
                'class' => 'btn bg-pink waves-effect',
                'onclick' => 'VAANCE_Users.loginAsUser("' . $this->entity->id . '")',
            ],
