...
abstract class ModuleCrudController extends AppBaseController
{
    ...
    /**
     * Tab buttons with assigned javascript action
     * @var array
     */
    protected $indexTabButtons = [];
    ...
    public function store()
    {
        ...
        $vowels = array(",", "€");
        if(isset($storeValues["car_p1_limit_amount"]) && isset($storeValues["car_p2_limit_amount"]) && isset($storeValues["car_p3_limit_amount"])) {
            for($i = 1; $i < 4; $i ++) {
                $storeValues["car_p".$i."_limit_amount"] = floatval(str_replace($vowels, "", $storeValues["car_p".$i."_limit_amount"]));
                $storeValues["car_p".$i."_premium_gross"] = floatval(str_replace($vowels, "", $storeValues["car_p".$i."_premium_gross"]));
                $storeValues["car_p".$i."_premium_net"] = floatval(str_replace($vowels, "", $storeValues["car_p".$i."_premium_net"]));
            }
        }
        ...
        if (config('vaance.record_assigned_notification_enabled')) {
    ...
    public function update($identifier)
    {
        ...
        $vowels = array(",", "€");
        if(isset($input["car_p1_limit_amount"]) && isset($input["car_p2_limit_amount"]) && isset($input["car_p3_limit_amount"])) {
            for($i = 1; $i < 4; $i ++) {
                $input["car_p".$i."_limit_amount"] = floatval(str_replace($vowels, "", $input["car_p".$i."_limit_amount"]));
                $input["car_p".$i."_premium_gross"] = floatval(str_replace($vowels, "", $input["car_p".$i."_premium_gross"]));
                $input["car_p".$i."_premium_net"] = floatval(str_replace($vowels, "", $input["car_p".$i."_premium_net"]));
            }
        }
    ...
    public function destroy($identifier)
    {
        ...
        if (!$forceDelete ) {
            ...
            if (config('vaance.validate_fk_on_soft_delete')) {
