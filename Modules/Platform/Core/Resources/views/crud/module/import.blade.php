                    ...
                    <form class="form-horizontal" method="POST" action="{{ route($routes['import_process'])  }}">
                        ...
                        <div class="table-responsive">
                            ...
                            <table class="table table-bordered table-striped">
                                ...
                                @endforeach
                                <tr>
                                    @foreach ($csv_data[0] as $valueKey =>  $value)
                                        <td>
                                            ...
                                            <select style="min-width: 250px" class="select2" name="fields[{{ $value }}]">
                                                ...
                                                @foreach ($module_fields as $mkey => $db_field)
                                                    ...
                                                    <option @if(similar_text($valueKey,trans($language_file.'.form.'.$db_field),$perc) && $perc > 60 ) selected="selected" @endif value="{{ $db_field }}">

                                                        @lang($language_file.'.form.'.$db_field) ({{ $db_field }})
