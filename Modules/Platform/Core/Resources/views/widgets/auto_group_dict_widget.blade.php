...
<div class="{{ $config['coll_class'] }}">
    ...
    <div class="info-box-2 bg-{{ $record['color'] }} hover-expand-effect {{ isset($config['dataTableToFilter']) && $config['dataTableToFilter']  != '' ? "filterable-group-widget pointer": '' }}" data-col-name="{{$config['groupBy']}}" data-name="{{$record['title']}}" >
        <div class="icon">
        ...
        <div class="content">
            ...
            <div class="number {{ !config('vaance.disableCountTo') ? 'count-to': '' }}" data-from="0" data-to="{{ $record['count'] }}" data-speed="2000" data-fresh-interval="20">{{ $record['count'] }}</div>
        </div>
