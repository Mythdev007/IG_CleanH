...
class InvoiceForm extends Form
{
    public function buildForm()
    {
        ...
        $this->add('invoice_number', 'text', [
            ...
            'default_value' => is_array($this->model) ? $invoiceService->generateInvoiceNumber() : ''
        ]);
        ...
        // TODO https://gitlab.com/laravelvaance/multicrm/issues/150
        $this->add('due_date', 'dateType', [
            'label' => trans('invoices::invoices.form.due_date'),
            'default_value' => is_array($this->model) ? UserHelper::formatUserDate(Carbon::now()->addDay( $invoiceService->getDefaultDueDays())->format('Y-m-d')) : ''
        ]);
        ...
        $this->add('is_recurring', 'checkbox', [
            'label' => trans('invoices::invoices.form.is_recurring'),
            'removeFromMassUpdate' => true,
        ]);
        $this->add('recurring_every', 'number', [
            'label' => trans('invoices::invoices.form.recurring_every'),
        ]);

        $this->add('send_invoice_to', 'text', [
            'label' => trans('invoices::invoices.form.send_invoice_to'),
        ]);
        $this->add('recurring_period_id', 'select', [
            'choices' => InvoiceRecurringPeriod::all()->pluck('name', 'id')->toArray(),
            'attr' => ['class' => 'select2 pmd-select2 form-control'],
            'label' => trans('invoices::invoices.form.recurring_period_id'),
            'empty_value' => trans('core::core.empty_select')
        ]);

        $this->add('invoice_status_id', 'select', [
            'choices' => InvoiceStatus::orderBy('name')->pluck('name', 'id')->toArray(),
            'attr' => ['class' => 'select2 pmd-select2 form-control'],
        ...
        $this->add('from_country_id', 'select', [
            'choices' => Country::whereIsActive(1)->pluck('name', 'id')->toArray(),
            'attr' => ['class' => 'select2 pmd-select2 form-control'],
            'label' => trans('invoices::invoices.form.from_country_id'),
            'empty_value' => trans('core::core.empty_select')
        ]);
        ...
        $this->add('bill_country_id', 'select', [
            'choices' => Country::whereIsActive(1)->pluck('name', 'id')->toArray(),
            'attr' => ['class' => 'select2 pmd-select2 form-control'],
            'label' => trans('invoices::invoices.form.bill_country_id'),
            'empty_value' => trans('core::core.empty_select')
        ]);
        ...
        $this->add('ship_country_id', 'select', [
            'choices' => Country::whereIsActive(1)->pluck('name', 'id')->toArray(),
            'attr' => ['class' => 'select2 pmd-select2 form-control'],
            'label' => trans('invoices::invoices.form.ship_country_id'),
            'empty_value' => trans('core::core.empty_select')
        ]);
