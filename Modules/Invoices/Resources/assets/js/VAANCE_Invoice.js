var VAANCE_Invoice = {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if(result.data.product_id > 0 ){
                    ...
                    var row = VAANCE_Invoice.tmpProductRow;
                    ...
                    VAANCE_Invoice.recalculateRows();
    ...
    searchPriceBook: function(){
        ...
        $(document).on('click','.search-price-list', function(e){
            ...
            VAANCE_Invoice.tmpPriceListRow = current.closest('.invoice_row');

            var row = VAANCE_Invoice.tmpPriceListRow;
            ...
            if(product.val() > 0 ){
                ...
                VAANCE_Common.showNotification('bg-red',$.i18n._('select_product_first'));
            }
        ...
        $(document).on('click', '#RelatedModalTable tbody a', function (e) {
            ...
            if(recordType === 'Modules\\Products\\Entities\\PriceList'){
                VAANCE_Invoice.loadPriceBook(recordId);
            }
        });
    ...
    searchProduct: function(){
        ...
        $(document).on('click','.search-product', function(e){
            ...
            VAANCE_Invoice.tmpProductRow = current.closest('.invoice_row');
        ...
        $(document).on('click', '#RelatedModalTable tbody a', function (e) {
            ...
            if(recordType === 'Modules\\Products\\Entities\\Product'){
                ...
                VAANCE_Invoice.loadProduct(recordId);
    ...
    copyButtons: function(){
        ...
        $(document).on('click','#invoice-copy-from-company',function(e){
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    $('#from_country_id').val(result.data.country_id);
        ...
        $(document).on('click','#invoice-copy-from-shipping',function(e){
            ...
            var ship_country = $("#ship_country_id :selected").val();
            ...
            $('#bill_country_id').val(ship_country);
            ...
            if(accountId > 0 ) {
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        $('#bill_country_id').val(result.data.country_id);
        ...
        $(document).on('click','#invoice-copy-from-billing',function(e){
            ...
            var bill_country = $("#bill_country_id :selected").val();
            ...
            $('#ship_country_id').val(bill_country);
    ...
    recalculateRows: function(){
        ...
        VAANCE_Invoice.recalculateSummary();
    },
    ...
    recalculateSummary: function(){
        ...
        var currencyValue = $('#currency_id').val();
        var currentyName = '';

        if(currencyValue != '') {
            currentyName = ' ' + $('#currency_id option:selected').html();
        }

        $.each($('.row_line_total'), function(index,value){

            lineTotal = $(value).html().replaceAll(',','').replaceAll('&nbsp;','').replace(/[a-z]/gi, '') ;

            subtotal += parseFloat(lineTotal);

            $(this).html($.number(lineTotal,2,'.',',')+currentyName);
        });

        var tax = VAANCE_Invoice.calculateTax(subtotal,discount,deliveryCost);

        var summaryGross = VAANCE_Invoice.calculateGross(subtotal,deliveryCost,discount,tax);
        var balanceDue = VAANCE_Invoice.calculateBalanceDue(summaryGross,paid);



        $('.summary_subtotal').html($.number(subtotal,2,'.',',')+currentyName);
        $('.summary_discount').html($.number(discount,2,'.',',')+currentyName);
        $('.summary_delivery_cost').html($.number(deliveryCost,2,'.',',')+currentyName);
        $('.summary_tax').html($.number(tax,2,'.',',')+currentyName);
        $('.summary_paid_to_date').html($.number(paid,2,'.',',')+currentyName);
        $('.summary_balance_due').html($.number(balanceDue,2,'.',',')+currentyName);
        $('.summary_gross').html($.number(summaryGross,2,'.',',')+currentyName);
